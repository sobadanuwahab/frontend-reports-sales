      ...
      if (error.response) {
        switch (error.response.status) {
          ...
          case 404:
            ...
            break;
          default:
            setError("Gagal mengambil data outlet. Silakan refresh halaman.");
        }
        ...
        <div style={styles.statsContainer}>
          {(() => {
            ...
            outlets.forEach((outlet) => {
              if (outlet.nama_outlet && outlet.nama_outlet.trim() !== "") {
                uniqueOutletNames.add(outlet.nama_outlet.toLowerCase().trim());
              }
            });
            const totalUniqueOutlets = uniqueOutletNames.size;

            console.log("Unique outlet names:", Array.from(uniqueOutletNames));
            console.log("Total unique outlets:", totalUniqueOutlets);
              ...
              if (outlet.nama_outlet && outlet.nama_outlet.trim() !== "") {
                ...
                if (!lob) {
                  ...
                  if (lobLower === "cafe" || lob === "Cafe") {
                    lobCounts["Cafe"]++;
                  } else if (lobLower === "premiere" || lob === "Premiere") {
                    lobCounts["Premiere"]++;
                  } else if (
            ...
            return (
              <>
                ...
                <div style={styles.statCard}>
                  <div style={styles.statValue}>{totalUniqueOutlets}</div>
                  <div style={styles.statLabel}>Total Outlet</div>
                </div>

                {/* Cafe - Hitung dari SEMUA DATA */}
                <div style={styles.statCard}>
                  <div style={styles.statValue}>{lobCounts["Cafe"] || 0}</div>
                ...
                <div style={styles.statCard}>
                  ...
                  <div style={styles.statLabel}>Hello Sunday</div>
                </div>
                ...
                {lobCounts["Tanpa LOB"] > 0 && (
                  <div style={styles.statCard}>
                    <div style={styles.statValue}>{lobCounts["Tanpa LOB"]}</div>
                    <div style={styles.statLabel}>Tanpa LOB</div>
                  </div>
                )}
              </>
            );
        ...
        <div style={styles.tableContainer}>
          {loading ? (
            ...
            <div style={styles.emptyState}>
              <FontAwesomeIcon icon={faStore} style={styles.emptyIcon} />
            ...
            <>
              <div style={styles.tableInfo}>
                <span>
                  Menampilkan <strong>{currentOutlets.length}</strong> dari{" "}
                  <strong>{filteredOutlets.length}</strong> outlet
                </span>
                <span>
              ...
              <table style={styles.table}>
                <thead>
                  <tr>
                    <th style={styles.tableHeader}>Kode Outlet</th>
                    <th style={styles.tableHeader}>Nama Outlet</th>
                    <th style={styles.tableHeader}>LOB</th>
                    <th style={styles.tableHeader}>Tanggal Dibuat</th>
                    <th style={styles.tableHeader}>Terakhir Diubah</th>
                      ...
                      <td style={styles.tableCell}>
                        {outlet.lob ? (
                          <span
                            style={{
                              ...styles.lobBadge,
                              backgroundColor: getLobColor(outlet.lob),
                            }}
                          >
                            {outlet.lob}
                          </span>
                        ) : (
                          <span style={styles.noLobText}>-</span>
                        )}
                      </td>
                      <td style={styles.tableCell}>
                          ...
                          <button
                            onClick={() =>
                              setDeleteModal({
                                show: true,
                                id: outlet.id,
                                name: outlet.nama_outlet || "Outlet ini",
                              })
                            }
          ...
          <div style={styles.modalContent}>
            ...
            <div style={styles.modalBody}>
              ...
              <p style={styles.warningText}>
                Tindakan ini tidak dapat dibatalkan. Semua data terkait outlet
                ini akan hilang.
              </p>
            </div>

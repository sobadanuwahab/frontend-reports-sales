...
const ReportCreate = () => {
  ...
  const fetchOutlets = async () => {
    try {
      ...
      if (!token) {
        setStatusMessage("Token tidak ditemukan. Silakan login kembali.");
        return;
      }
      ...
      const allOutlets = outletsData.map((outlet) => ({
        ...
        nama_outlet:
          outlet.nama_outlet || outlet.nama || outlet.name || "No Name",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        omzet_fnb: roundForDatabase(parseFloat(formData.omzet_fnb || 0), 2),
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.message || "Gagal membuat laporan");
      }
    } catch (error) {
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
            ...
            for (let rowIndex = 0; rowIndex < batch.length; rowIndex++) {
              ...
              try {
                ...
                const date = getValue(row, ["tanggal", "date", "report_date"]);
                ...
                if (kodeOutlet && normalizedLob) {
                  outlet = outlets.find(
                    (o) =>
                      o.kode_outlet &&
                      o.lob &&
                      o.kode_outlet.toString().trim() === kodeOutlet.trim() &&
                      normalizeLob(o.lob) === normalizedLob,
                  );
                  if (outlet) searchMethod = "kode_lob_normalized";
                }
                ...
                if (!outlet && kodeOutlet && lob) {
                  outlet = outlets.find(
                    (o) =>
                      ...
                      o.lob &&
                      o.kode_outlet.toString().trim() === kodeOutlet.trim() &&
                      o.lob.toLowerCase().trim() === lob.toLowerCase().trim(),
                  );
                  if (outlet) searchMethod = "kode_lob_case_insensitive";
                }
                  ...
                  outlet = outlets.find(
                    (o) =>
                      (o.kode_outlet &&
                        o.lob &&
                        o.kode_outlet.toString().trim() === kodeOutlet.trim() &&
                        normalizeLob(o.lob).includes(normalizeLob(lob))) ||
                      normalizeLob(lob).includes(normalizeLob(o.lob)),
                  );
                  if (outlet) searchMethod = "kode_lob_partial";
                }
                ...
                if (!outlet && kodeOutlet) {
                  outlet = outlets.find(
                    (o) =>
                      o.kode_outlet &&
                      o.kode_outlet.toString().toLowerCase().trim() ===
                        kodeOutlet.toLowerCase().trim(),
                  );
                  if (outlet) searchMethod = "kode_case_insensitive";
                }
                ...
                console.log(`✓ Outlet ditemukan via ${searchMethod}:`, outlet);
                ...
                if (!cleanDate) {
                  ...
                  if (isNaN(dateObj.getTime())) {
                    validationErrors.push(`Format tanggal salah: ${date}`);
                  }
                }
                ...
                const cleanTargetPercentage = cleanNumber(targetPercentage);
                ...
                if (isNaN(parseInt(totalBills.replace(/\D/g, "")))) {
                  validationErrors.push(
                    `Jumlah Bill bukan angka: ${totalBills}`,
                  );
                }
                if (isNaN(parseInt(totalAudience.replace(/\D/g, "")))) {
                  validationErrors.push(
                ...
                targetPercentNum = parseFloat(cleanTargetPercentage);
                ...
                const payload = {
                  ...
                  omzet_fnb:
                    roundForDatabase(parseFloat(cleanOmzetFnb), 2) || 0,
                  omzet_cinema:
                    roundForDatabase(parseFloat(cleanOmzetCinema), 2) || 0,
                  total_bills: parseInt(totalBills.replace(/\D/g, "")) || 0,
                  total_audience:
                    parseInt(totalAudience.replace(/\D/g, "")) || 0,
                  target_percentage:
                    preserveTargetPercentage(targetPercentNum) || 0,
                  target_head: parseInt(targetHead.replace(/\D/g, "")) || 0,
                  notes: notes || "",
                };

                // **4. Send to API dengan retry logic untuk rate limiting**
                const sendRequestWithRetry = async (retryCount = 0) => {
                  try {
                    const response = await axios.post(
                      `${API_BASE_URL}/reports`,
                      payload,
                    ...
                    if (response.data.success) {
                      ...
                      throw new Error(response.data.message || "Gagal import");
                    }
                  } catch (error) {
              ...
              // Update progress
              const progress = Math.round((absoluteRowIndex / totalRows) * 100);
          ...
          setStatusMessage(`❌ Gagal memproses file: ${error.message}`);
          setSubmitStatus("error");
          setShowImportError(true);
        } finally {
  ...
  const calculateDerivedValues = () => {
    ...
    return {
      ...
      achievementRate: targetHead > 0 ? (totalAudience / targetHead) * 100 : 0,
    };
  };
  ...
  return (
    <div style={styles.container}>
      <div style={styles.content}>
        ...
        {submitStatus === "success" && statusMessage && !showImportModal && (
        ...
        {submitStatus === "error" && statusMessage && !showImportModal && (
          ...
          <div style={styles.formGrid}>
            ...
            <div style={styles.formGroup}>
              ...
              {isLoadingOutlets ? (
                ...
                <div style={styles.noDataSelect}>Tidak ada data outlet</div>
              ) : (
                <select
                  name="outlet_id"
                ...
                >
                  ...
                  {(() => {
                    ...
                    outlets.forEach((outlet) => {
                      ...
                      grouped[outlet.nama_outlet].push(outlet);
                    });

                    return Object.keys(grouped).map((nama) => (
                      <optgroup key={nama} label={nama}>
                        {grouped[nama].map((outlet) => (
                          <option
              ...
              {errors.outlet_id && (
                <p style={styles.errorText}>{errors.outlet_id}</p>
              )}
            </div>
            ...
            <div style={styles.formGroup}>
              ...
              />
              {errors.report_date && (
                <p style={styles.errorText}>{errors.report_date}</p>
              )}
            </div>
          </div>
          ...
          <div style={styles.modalOverlay}>
            <div style={styles.modal}>
              ...
              <div style={styles.modalBody}>
                ...
                {submitStatus && (
                  <div
                    style={{
                      ...
                      borderColor:
                        submitStatus === "success" ? "#a7f3d0" : "#fca5a5",
                    }}
                  >
                    <FontAwesomeIcon
                      icon={
                ...
                {submitStatus === "success" && importSuccessDetails && (
                  <div style={styles.modalSuccessAlert}>
                    <div style={styles.modalSuccessContent}>
                      <FontAwesomeIcon
                        icon={faCheckCircle}
                        style={styles.modalSuccessIcon}
                      />
                      <div style={styles.modalSuccessText}>
                        <h4 style={styles.modalSuccessTitle}>
                          Import Berhasil! 🎉
                        </h4>
                        <p style={styles.modalSuccessMessage}>
                          {importSuccessDetails.message}
                        </p>
                        <div style={styles.modalSuccessStats}>
                          <div style={styles.modalSuccessStat}>
                            <span style={styles.modalStatLabel}>Total:</span>
                            <span style={styles.modalStatValue}>
                              {importSuccessDetails.totalRows} baris
                            </span>
                          </div>
                          <div style={styles.modalSuccessStat}>
                            <span style={styles.modalStatLabel}>Berhasil:</span>
                            <span
                              style={{
                            ...
                            >
                              {importSuccessDetails.successCount} data
                            </span>
                          </div>
                          <div style={styles.modalSuccessStat}>
                            <span style={styles.modalStatLabel}>Gagal:</span>
                            <span
                              style={{
                                ...styles.modalStatValue,
                                color: "#ef4444",
                            ...
                            >
                              {importSuccessDetails.errorCount} data
                            </span>
                          </div>
                        </div>
                        <p style={styles.modalSuccessNote}>
                ...
                <div style={styles.templateInfo}>
                  ...
                  <p>File harus memiliki kolom berikut (format fleksibel):</p>
                  ...
                  <div style={styles.templateFormat}>
                    ...
                    <ul style={styles.templateList}>
                      ...
                      <li>
                        <strong>Nama Outlet</strong> - Nama outlet (opsional
                        jika ada kode outlet)
                      </li>
                      <li>
                      ...
                      <li>
                        ...
                        Line of Business (CAFE, PREMIERE, HS)
                      </li>
                      <li>
                      ...
                      <li>
                        ...
                        Angka 0-100 (contoh: 68.34 untuk 68.34%)
                        <br />
                        <span
                          style={{
                            color: "#3b82f6",
                        ...
                        >
                          * Nilai akan disimpan persis seperti di Excel (tanpa
                          konversi)
                        </span>
                      </li>
                      <li>
                      ...
                      <li>
                        <strong>Catatan</strong> - Teks (opsional)
                      </li>
                    </ul>

                    <h5>🏪 Contoh Data Outlet di Sistem:</h5>
                    <div style={styles.outletsExample}>
                      <table style={styles.exampleTable}>
                        ...
                        <tbody>
                          {outlets.slice(0, 5).map((outlet, idx) => (
                            <tr key={idx}>
                              <td>
                                <strong>{outlet.kode_outlet}</strong>
                              </td>
                      ...
                      {outlets.length > 5 && (
                        ...
                        >
                          ... dan {outlets.length - 5} outlet lainnya
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <div style={styles.uploadArea}>
                  <label style={styles.uploadLabel}>
                    <input
                      ...
                      disabled={isImporting || submitStatus === "success"}
                    />
                    <div style={styles.uploadContent}>
                      ...
                      <p style={styles.uploadHint}>.xlsx, .xls, .csv</p>
                    </div>
                  </label>
                </div>
                ...
                {importPreview.length > 0 && (
                  <div style={styles.previewContainer}>
                      ...
                      <table style={styles.previewTable}>
                        ...
                        <tbody>
                          {importPreview.map((row, index) => (
                            <tr key={index}>
                              <td>{row.kodeOutlet}</td>
                              <td>{row.namaOutlet}</td>
                              <td>{row.date}</td>
                              <td>{row.lob}</td>
                              <td>Rp {formatNumber(row.omzetFnb)}</td>
                              <td>
                                <span
                                  style={{
                                    color: "#3b82f6",
                    ...
                    <div style={styles.previewStats}>
                      <span>Total baris: {importPreview.length}</span>
                      <span>
                        Status:{" "}
                        {submitStatus === "success"
                          ? "Import Selesai"
                ...
                {isImporting && (
                  <div style={styles.progressContainer}>
                    ...
                    <div style={styles.progressText}>
                      Memproses... {importProgress}% ({importSuccessCount}{" "}
                      berhasil, {importErrorCount} gagal)
                    </div>
                    <div style={styles.progressDetails}>
                      ...
                      <div style={styles.progressStat}>
                        ...
                        >
                          📊 Total: {importSuccessCount + importErrorCount}
                        </span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div style={styles.detailsContainer}>
                      ...
                      <div style={styles.detailsList}>
                        {importDetails.map((detail, index) => (
                          ...
                          >
                            Baris {detail.row}: {detail.message}
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                {submitStatus === "success" && (
                  ...
                  >
                    ...
                    >
                      ...
                      <span>
                        Modal akan tertutup otomatis dalam beberapa detik...
                      </span>
                    </p>
                  </div>
                )}
              ...
              <div style={styles.modalFooter}>
                <button
                  ...
                  disabled={isImporting}
                >
                  {submitStatus === "success" ? "Tutup Sekarang" : "Batal"}
                </button>
                <button
                  ...
                  disabled={
                    !importFile || isImporting || submitStatus === "success"
                  }
                >
                  {isImporting ? (
                    <>

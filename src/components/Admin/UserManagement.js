...
const UserManagement = () => {
    ...
    const filteredUsers = users.filter((userItem) => {
        const matchesSearch =
            (userItem.name?.toLowerCase() || "").includes(
        ...
        const matchesRole =
            filterRole === "all" || userItem.role === filterRole;
    ...
    const handleResetPassword = async (userId) => {
        if (
            window.confirm("Apakah Anda yakin ingin mereset password user ini?")
        ) {
            try {
                const token = localStorage.getItem("token");
                const config = {
                ...
                if (response.data.success) {
                    alert(
                        `Password berhasil direset ke: ${
                            response.data.data?.default_password ||
                            "password123"
                        }`
                    );
                } else {
                    ...
                    <div style={styles.headerLeft}>
                        ...
                        <div style={styles.headerContent}>
                            ...
                            <p style={styles.subtitle}>
                                Kelola semua user dari tabel users
                            </p>
                        </div>
                    </div>
